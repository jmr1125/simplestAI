...
#define Init net.last_layer()->init(std::move(rd))
    ...
    if_netin_load(matrix_layer) else {
      net.add_matrix_layer({1, 1}, 28 * 28, 512);
      Init;
    }
    ...
    if_netin_load(matrix_layer) else {
      net.add_matrix_layer({1, 1}, 512, 512);
      Init;
    }
    ...
    if_netin_load(matrix_layer) else {
      net.add_matrix_layer({1, 1}, 512, 128);
      Init;
    }
  ...
  for (; !quit;) {
    ...
    // }
    {
      ...
      for (int j = 0; j < instance[off].second.size(); ++j) {
        ...
        // pow(instance[off].second[j] - out[j], 2);
      }
    ...
    auto g =
        net.update(instance[off].first, instance[off].second, train_method::l2);
